...
@Composable
fun SectionChild3Screen(
    navController: NavController,
    viewModel: SectionChild3ViewModel = hiltViewModel()
) {
                ...
                Child3ListItem(
                    ...
                    onClick = {
                        navController.navigate(
                            Screen.SectionChild3Sub.createRoute(viewModel.sectionHeadAddedId, item.id)
                        )
                    },
                ...
                Row(
                    modifier = Modifier
                        ...
                        .clickable(
                            ...
                            onClick = {
                                navController.navigate(
                                    Screen.SectionChild3Sub.createRoute(viewModel.sectionHeadAddedId, -1)
                                )
                            }
                        )
                        .padding(16.dp),
    ...
    deleteTarget?.let { target ->
        ...
        AlertDialog(
            ...
            text = { Text(stringResource(R.string.msg_delete_entry_confirm, target.studyDegree.ifEmpty { target.schoolName }.ifEmpty { thisEntry })) },
            confirmButton = {
...
private fun Child3ListItem(
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            val primary = item.studyDegree.ifEmpty { item.schoolName }
            ...
            if (item.schoolName.isNotEmpty() && item.studyDegree.isNotEmpty())
                Text(item.schoolName, style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant)
            if (item.studyPeriod.isNotEmpty())
                Text(item.studyPeriod, style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
        Box {
